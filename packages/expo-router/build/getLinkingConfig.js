...
async function getLinkingConfig(store, routes, context, { metaOnly = true, serverUrl } = {}) {
    ...
    const getStateFromPath = linking_1.getStateFromPath.bind(store);
    return {
        prefixes: [],
        config,
        ...
        getActionFromState(state, options) {
            let action = (0, native_1.getActionFromState)(state, options);
